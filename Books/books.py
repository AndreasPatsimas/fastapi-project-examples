...
class Book(BaseModel):
    ...
    description: Optional[str] = Field(min_length=1, max_length=100, title="Description of Book")
...
async def negative_number_exception_handler(request: Request, exception: NegativeNumberException):
    return JSONResponse(status_code=418, content={"message": "Why do you want {} books? You need to read more"
                        .format(exception.books_to_return)})


@app.post("/books/login/")
async def book_login(book_list_id: int, username: Optional[str] = Header(None), password: Optional[str] = Header(None)):
    if username == "FastAPIUser" and password == "test1234!":
        return BOOKS[book_list_id]
    return "Invalid User"
...
@app.get("/")
async def all_books(books_to_return: Optional[int] = None):
...
async def find_book_no_rating(book_id: UUID):
    for book in BOOKS:
        if book.id == book_id:
            return book
    raise raise_item_cannot_be_found_exception()
...
async def create_book(book: Book):
    book.id = uuid.uuid4()  # generate random UUID
    BOOKS.append(book)
    return book
